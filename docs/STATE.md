...
`plugins/hard-update-restart/` now offers **Update everything and hard-restart** (`all`, v0.3.0) and a dedicated **Hard-restart Herdr (no updates)** action (`restart`), supporting both local-only and multi-machine fleet restarts across Herdr mesh machines (e.g. spark0, mbp-16-24, emo-win). A pure hard-restart mode stops pane processes, restarts Herdr servers, and restores agent sessions into fresh runtime processes without updating runtimes or plugins, allowing instant config/model reloads. When updates are enabled, custom and symlinked Herdr binaries are automatically protected from being overwritten by `herdr update` (checked locally via `lstatSync` and remotely via `test -L`, or overridden with `HERDR_SKIP_SELF_UPDATE=1`). Discovered machines can be updated and hard-restarted concurrently over SSH with target-isolated logging, remote server lifecycle management, safe native session recovery without host-local path assumptions, and aggregated completion status. It shares the Herdr-plugin updater engine with `plugins/plugin-updater/` v0.1.3. A snapshotted detached worker manages drainage and replacement. All package typechecks and 38 tests pass.
...
`plugins/ticket-bound-attention/` v0.1.0 captures a Linear ticket for [EMO-426](https://linear.app/emo-eth/issue/EMO-426/ticket-bound-attention). `capture` binds GOAL.md / `.herdr-ticket` or creates a team EMO issue with Intention, Vibe, Done-when, and Map. `worktree.created` and `worktree.opened` run the same path. Titles are human names; ticket-number chairs are refused. Workspace metadata token `ticket` is set. Linked and enabled locally.
...
| Ticket-bound attention capture | `docs/ticket-bound-attention/vibe.md`, `docs/log/2026-09-18-ticket-bound-implementation-plan.md`, EMO-426 | `plugins/ticket-bound-attention/` | package `npm test` (17/17), `npm run check`; live `worktree.created` / `worktree.opened`; disposable bind [EMO-453](https://linear.app/emo-eth/issue/EMO-453/disposable-capture-proof) then `workspace close` | verified-live for named macOS scenarios |
