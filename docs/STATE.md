...
Focused evidence for v0.2: strict TypeScript compilation, 26 Node host tests, 20 Python contract tests, and real Pi 0.84.1 and OMP 17.2.15 native-runner execution. The isolated CLI auth smoke returned a truthful missing state without exposing a secret. Five independent correctness, security, performance, content-contract, and auth-contract reviewers found and verified fixes for all material findings; their final post-fix verdicts were clean. A post-audit correction preserves truthful refreshability for expired subscription credentials and lets a twice-rejected rotated source fall through to the next subscription without exposing billed API access. Historic v0.1 live OMP evidence remains in `docs/log/2026-08-28-grok-search-tools.md`; the new structured retrieval shape has focused and native-host proof but has not spent a live xAI request because no supported credential is available to the new process without restarting OMP [verified-focused].
