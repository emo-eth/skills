...
`plugins/hard-update-restart/` now offers **Update everything and hard-restart** (`all`, v0.3.0), supporting both local-only and multi-machine fleet restarts across Herdr mesh machines (e.g. spark0, mbp-16-24, emo-win). Discovered machines can be updated and hard-restarted concurrently over SSH with target-isolated logging, remote server lifecycle management, safe native session recovery without host-local path assumptions, and aggregated completion status. It shares the Herdr-plugin updater engine with `plugins/plugin-updater/` v0.1.3. A snapshotted detached worker updates runtimes/extensions/plugins while the server remains available, drains agents before restart, and replaces only confirmed sessions. Missing or duplicate native session references prevent shutdown; partial-update failures are not reported as success. All package typechecks and 36 tests pass.
