...
The shared library now contains 437 canonical skills under `~/.agents/skills`: 436 explicit-only, with `herdr` the sole model-invokable skill. `AGENTS.md` records the `universal` installer target; matching metadata is published for the fifteen installed skills owned here, including wall-clock. Fresh OMP/Pi loaders verified canonical-only active sources and `herdr`-only automatic invocation; explicit OMP expansion also passed. Codex's native `skills/list` verified 437 active canonical skills with no load errors; its bundled source, 35 original plugin skill entries, and three embedded reference copies are disabled independently of plugin tools. Claude has no native shared-root discovery. Hermes remains unsupported by explicit user decision: it ignores invocation metadata and still recreates its essential skill under `~/.hermes/skills` despite the external-root and bundled-opt-out settings. Restart existing processes to discard loaded catalogs.
