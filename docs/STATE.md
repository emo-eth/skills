...
Turn-summary v1 is implemented on branch `turn-receipt` at `plugins/turn-summary/`. Its Pi and OMP native adapters use the same `context` event seam as wall-clock to append one fixed end-of-turn reminder capped at 400 words; `/summary on|off` uses the optional native command seam. The package has no model calls, UI, or MCP surface. It is installed and enabled in the normal local Pi and OMP profiles. Unsupported hosts fail closed. Focused Node and pinned OMP native-runner tests pass, and clean installed-plugin Pi and OMP evidence is recorded in `docs/log/2026-08-13-turn-summary.md`; this branch is not pushed or merged.
...
| Turn summaries | `docs/DECISIONS.md` D37, D38, `docs/vibe.md` companion clause | `plugins/turn-summary/` | `docs/log/2026-08-13-turn-summary.md`, package checks, installed-plugin Pi and OMP live evidence | verified-live |
