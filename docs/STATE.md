...
The same directory is now also an installable Pi and OMP Agent Plugin. Thin native adapters register `grok_search`, `grok_fetch`, and `grok_prompt`; one shared TypeScript runner validates source-specific inputs, executes the bundled Python CLI without a shell, forwards host cancellation, and returns structured citation data. Focused Node tests cover both adapters and the runner; pinned Pi 0.84.1 and OMP 17.2.15 native runners load and execute the tool seam. A clean OMP 18.0.5 process invoked `grok_prompt` through xAI successfully. Global Pi 0.84.3 lacks an authenticated calling model, so live Pi provider-level invocation remains a GAP. Contract and evidence: `docs/log/2026-08-28-grok-search-tools.md` [verified-live OMP; verified-focused Pi].
The default OMP profile and Pi user package list now reference pushed commit `6ee3449`. A fresh OMP 18.0.5 process discovered that installed extension without an explicit path and completed a real `grok_prompt` call; the already-running OMP process still needs a full restart before its tool list changes.
OMP code mode is compatible without another adapter: the `eval` bridge exposes installed tools as `tool.grok_search`, `tool.grok_fetch`, and `tool.grok_prompt`. A fresh OMP 18.0.5 JavaScript eval completed a real nested `grok_prompt` call and returned `GROK_CODE_MODE_OK` [verified-live].
...
The repository now publishes `skills/herdr-voice/SKILL.md`, a self-contained Codex realtime voice workflow that uses direct external Herdr CLI control with one exact named session and explicit targets. It keeps a Codex-in-Herdr relay as a restart-aware fallback. No Herdr, Codex, Pi, or OMP runtime change is required [documented].
...
| Herdr voice control | `skills/herdr-voice/SKILL.md`, `docs/log/2026-08-30-herdr-skill-management.md` | delegation-first: the Codex voice task dispatches written tasks to text subagents that own every `herdr` invocation; the voice agent never runs `herdr` itself | reinstall from origin/main plus contract inspection | verified-live |
