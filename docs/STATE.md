...
| Herdr voice control | `skills/herdr-voice/SKILL.md`, `docs/log/2026-08-30-herdr-skill-management.md` | direct external CLI control with exact `--session` and explicit targets; a Codex-in-Herdr relay is the fallback when direct thread control is unavailable | reinstall from origin/main, source comparison, and installed-file checks | verified-live |
