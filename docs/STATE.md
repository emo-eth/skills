...
The repository now also contains `plugins/wranglr/` (renamed from `focus-order` on 2026-08-14), a Herdr plugin that stores ranked agent and worktree identities, focuses the highest-ranked urgent target, or opens a separate attention popup. It uses one-shot startup and event hooks, an atomic attention-owner marker, and optional Pi and OMP companion adapters. The manager pane anchors its selection to the first row on open, clamps arrow movement at both ends, keeps the selected row visible when content overflows the popup, and sorts the agent table by rank, status, agent, project, or worktree with the `t`/`T` keys (`src/herdr/selection.ts`, `src/herdr/table-sort.ts`). The implementation is verified in the isolated skills worktree with TypeScript compilation, 125 Node tests, a live PTY smoke run against the local Herdr socket, local Herdr manifest linking, and action discovery; the package is not yet published from the canonical checkout. The rename is a clean cutover: the state file moved to `~/.config/herdr/wranglr/wranglr.json` and the registered agent command is now `wranglr`, with no migration shim.
...
| Wranglr plugin (agent focus order) | none yet | `plugins/wranglr/` | `npm run check`, 125 Node tests, `herdr plugin link`, and `herdr plugin action list` | verified-focused |
