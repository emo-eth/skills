...
The repository also includes `plugins/linear-command/` v0.1.0 (`@emo-eth/linear-command-plugin`), an OMP and Pi command plugin that registers `/linear` for out-of-band issue creation. It captures ambient workspace context (cwd, repo, git branch, session metadata), maps Linear taxonomy (team `EMO`, projects, review buckets 01-10, labels, priorities), prompts interactively via UI dialogs when available, and invokes `linear issue create` via the local Linear CLI without injecting conversation entries into the primary session transcript. Package checks and tests (`node:test` unit suite and native OMP `ExtensionRunner` integration) verify command registration, taxonomy mapping, preview flows, and execution.
