...
User/global skill installs now use only `~/.agents/skills`, with `herdr` the sole model-invokable skill. The installation contract and `universal` installer target are in `AGENTS.md`; matching metadata is published for the fifteen installed skills owned here, including the wall-clock plugin skill. Fresh OMP and Pi loader runs verified canonical-only active sources, `herdr`-only automatic invocation, and explicit OMP skill expansion. Codex's bundled source and original plugin skill entries are disabled independently of plugin tools. Claude has no native shared-root discovery; Hermes's invocation controls remain unsupported by explicit user decision. Existing processes must restart to discard loaded skill catalogs.
...
| --- | --- | --- | --- | --- |
| Global skill policy | `AGENTS.md` installation contract | installed-source `SKILL.md` frontmatter and `agents/openai.yaml`; wall-clock plugin skill | native OMP/Pi discovery and explicit OMP expansion; published metadata checks; focused wall-clock package checks | verified-live for named checks; Hermes unsupported |
