...
The installed Herdr binary owns the live `herdr` skill and exposes release-matched instructions through `herdr --skill`. The global skill lock and copied universal skill directory exclude `herdr`; chezmoi's lock merger removes stale entries and copied directories when it encounters legacy lock metadata during apply or update. This repository retains `skills/herdr/SKILL.md` as a published reference fork, not the live runtime copy. `skills/herdr-voice/SKILL.md` delegates voice requests to one OMP process, which reads the binary-served skill and follows its refusal when outside a Herdr pane. No Herdr, Codex, Pi, or OMP runtime patch is required [verified-focused].
...
| Herdr voice control | `skills/herdr-voice/SKILL.md`, `docs/log/2026-08-30-herdr-voice-delegation.md`, `docs/log/2026-08-30-herdr-skill-management.md` | delegation-only: the voice task starts one OMP delegate, hands it the whole request, and the delegate reads `herdr --skill`; an outside-pane refusal is relayed instead of bypassed | binary skill output and handoff contract inspection | verified-focused |
| Binary-managed Herdr skill | `skills/herdr/SKILL.md`, `docs/log/2026-08-30-herdr-skill-management.md` | the installed binary's `herdr --skill` output is authoritative; the local fork remains a published reference and is absent from universal installation and lock metadata | live binary output, stale-lock tombstone fixture, and real plugin-sync smoke | verified-live |
