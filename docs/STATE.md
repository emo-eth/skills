...
The approved grok-search north star is `docs/prds/2026-09-02-grok-search/vibe.md`. It defines two core modes: faithful direct retrieval of X posts, authored threads, and X Articles with expandable ecosystem context; and adaptive, representative X discovery and synthesis with bounded sentiment and recoverable evidence. Authentication is an enabling quality: reuse supported subscription authorization, offer agent-operated device approval when missing, re-resolve credentials without host restarts, and never turn subscription exhaustion into automatic API billing. Current code only partially satisfies this contract; the vibe is the upstream source for subsequent PRD and implementation work [documented, approved via Plannotator 2026-09-02].
...
| Grok-search north star | `docs/prds/2026-09-02-grok-search/vibe.md` | Current implementation only partially satisfies the approved retrieval, discovery, and frictionless-auth north star; downstream PRD/spec work has not started | explicit user approval via tailnet Plannotator on 2026-09-02 | documented |
