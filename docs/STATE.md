...
Current phase: v0 implementation complete on `main`. Pi 0.84.1 and OMP 17.2.15 load the native adapters, inject measured time, block late native work, and abort their real bash executors under `abort-running`. The native `/wallclock` command accepts an optional `start`, defaults an omitted policy to `block-new`, and submits an optional trailing prompt only after activation; idle use starts a turn and active use steers it. The host status display recalculates from the current clock once per second. OMP's real `TaskTool` creates tested children that inherit assignment context, block late work, report, terminate through native `yield`, and abort a running child bash action. Active phases and fast lanes now encourage one bounded delegated assignment when it can reduce uncertainty or finish independent work faster; wrap-up and expiry still block new delegation. The adapters also recognize explicit `/do-it-now` skill invocations and apply a fixed two-minute host guard.
...
Deferred: Codex and Claude activation [D10] until an open, tested enforcement seam exists; Claude proprietary systems; provider-specific remote cancellation; and a portable visual dashboard. Revisit D10 when v1 host support is scoped and revisit the last two when provider or user-interface requirements become active. From the vibe round: lost-chat recovery via session-history search [D24] and loop-duration recording [D25], revisit triggers in `docs/DECISIONS.md`. The full contract remains in `docs/prds/2026-08-11-wall-clock/prd.md`.
...
On the `sieve-vibe` branch, `docs/vibe.md` is a draft repo-level philosophy contract (progress through sifting: fast filter passes, few crystallization stages, deliverable breakdown, symbiotic understanding, recorded judgment, timed loops, in-the-moment friction logs, plus a companion turn-receipt clause). Two review rounds are applied and captured: round 1 (36 items, D17-D26) and round 2 (8 items, D27-D31, superseding D19 and D23), with answers in `docs/review/2026-08-13-vibe-round-1-answers.md` and `...-round-2-answers.md`. Round 2 established the vibe as the source of truth with skills and the artifact chain as downstream facets [D27, D30]. Proposals derived from the original dump are in `docs/log/2026-08-13-sieve-vibe.md` (its review session is open, no feedback yet). Open: the name of the proposed skill-scoped logger (not "yearn" [D21]); whether the receipt clause stays inside vibe.md or splits out; a proposed lc-ticketize/standup overlap audit awaiting user go-ahead; and, gated on vibe approval, revising lc-north-star and `docs/lifecycle.md` to declare the vibe upstream [D27].
...
| Repo philosophy (the sieve) | `docs/vibe.md`, `docs/log/2026-08-13-sieve-vibe.md`, `docs/review/2026-08-13-vibe-round-1-answers.md`, `docs/review/2026-08-13-vibe-round-2-answers.md` | not implemented; proposals P1-P5 pending review, P1/P4 reshaped by D28/D21 | Plannotator rounds 1-2 applied (36 + 8 items, D17-D31); approval pending | proposed |
