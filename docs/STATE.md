...
`plugins/hard-update-restart/` now offers one **Update everything and hard-restart** action (`all`, v0.2.0), sharing the Herdr-plugin updater engine with `plugins/plugin-updater/` v0.1.3. A snapshotted detached worker updates runtimes/extensions/plugins while the server remains available, then drains agents again and replaces only the confirmed session. Missing or duplicate native session references prevent shutdown; partial-update failures are not reported as success. Both package typechecks and 31 tests pass. An isolated macOS Herdr 0.9.0 run proved helper survival, real GitHub plugin upgrade with disabled/config preservation, and the same OMP conversation returning in a new process with a changed config marker. OMP/Pi downloads were fixture-backed; no production agent restart or actual Herdr version change is claimed. Usage and verification boundaries are in `plugins/plugin-updater/README.md`.
...
The repository also includes `plugins/antigravity-prompt/` v0.1.0 (`@emo-eth/antigravity-prompt-plugin`), an OMP-only plugin pinned to development host OMP 18.1.16 on Node >= 22.6. It rewrites the exact RFC 2119 normative sentence in OMP's system prompt to plain English to eliminate false 429 quota rejections on Google Antigravity accounts, preserving segmented string arrays without flattening and leaving original native tools untouched. It re-registers the `google-antigravity` provider under a custom API (`google-gemini-cli-prompt-fix`) using durable source ID `antigravity-prompt-fix-durable` so registrations survive child SDK extension-source cleanup, delegating directly to original `streamSimple` with detached models and preserving original OAuth JSON serialization. Verified live via user-scoped global link (`omp plugin link plugins/antigravity-prompt --scope user`) and clean OMP 18.1.16 process execution with Gemini 3.8 Flash high: parent and native scout/task children sequentially read repository targets across both worktree and canonical directories with zero 429s, no provider fallback, and no `--extension` or `--system-prompt` flags.
...
| Antigravity prompt fix | Handoff and design contracts | `plugins/antigravity-prompt/src/`, `plugins/antigravity-prompt/tests/` | Package check/test, live parent/scout/task smoke across worktrees on OMP 18.1.16 | verified-live |
...
| Combined updates and restart | `plugins/plugin-updater/README.md` | `plugins/hard-update-restart/`, shared `plugins/plugin-updater/src/core.ts` | 31 Node tests, both typechecks; isolated native Herdr restart, GitHub plugin upgrade, disabled/config preservation, same OMP session and new-process config load | verified-live for named macOS scenarios; runtime downloads fixture-backed |
