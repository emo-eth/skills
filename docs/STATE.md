...
The repository also includes `plugins/linear-command/` v0.1.0 (`@emo-eth/linear-command-plugin`), an OMP and Pi command plugin that registers `/linear` for out-of-band issue creation. It captures ambient workspace context (cwd, repo, git branch, session metadata), routes Linear taxonomy (team `EMO`, projects, review buckets 01-10, labels, priorities) through an out-of-band triage agent with explicit flag overrides, prompts interactively via UI dialogs when available, and invokes `linear issue create` via the local Linear CLI without injecting conversation entries into the primary session transcript. Package checks and tests (`node:test` unit suite and native OMP `ExtensionRunner` integration) verify command registration, agent routing, preview flows, and execution.
...
| Linear issue command | EMO-425, `GOAL.md` | `plugins/linear-command/` | package `npm run check`, `npm test` (node:test + native OMP runner) | verified-live |
