...
The repository now also publishes `skills/grok-search/`, a model-invoked skill with a stdlib-only python3 CLI (`scripts/grok-search.py`) that searches X (Twitter) and the web through xAI's server-side `x_search`/`web_search` Responses-API tools. It authenticates with the standalone `grok` CLI's subscription OAuth token in `~/.grok/auth.json` (`XAI_API_KEY` overrides), and it never consumes the single-use refresh token itself: on expiry it runs a minimal `grok -p` call so the grok CLI rotates its own tokens. Citations are deduplicated by post ID and capped in markdown output (`--max-citations`, full list in `--json`). Live-verified on this device: `x`, `fetch`, `web`, `ask`, and `auth` subcommands returned cited real-time results [verified-live].
...
| X/web live search | `skills/grok-search/SKILL.md` | `skills/grok-search/scripts/grok-search.py` | live `x`/`fetch`/`web` calls against api.x.ai with grok CLI OAuth, plus client-side validation error checks | verified-live |
