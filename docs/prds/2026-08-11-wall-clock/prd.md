...
- Requirement: Activation must carry one of two expiry policies: `block-new`, which enforces the parent deadline by rejecting new parent work while admitted direct parent work may finish, or `abort-running`, which enforces the parent deadline by rejecting new parent work and aborting every wall-clock-owned running parent action at expiry. Every child assignment is separately hard-bounded by the earlier of its requested budget and the parent hard deadline; the host must abort running child actions at child or parent expiry under either parent policy. The host must reject child work when it cannot prove a tested child abort path.
- Rationale: Child work must not outlive the parent time contract. The parent policy controls direct parent work; child assignments always inherit the parent's hard stop.
- Acceptance: The selected policy is visible in status and reports. An omitted native slash-command policy produces `block-new`. `block-new` never claims that a direct parent action stopped, but it still stops running child actions at the inherited parent deadline. `abort-running` emits and observes an abort signal for every owned running parent or child action, or rejects the action before it starts when cancellation cannot be enforced.
- Not acceptable: The product uses an undocumented default, silently changes an active policy, reports an in-flight action as cancelled without an observed abort, allows a child to continue past its child or parent deadline, or allows an unabortable child action.
