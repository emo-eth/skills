...
The shared `HostEnforcement` boundary in `src/host.ts` already separates policy from host capabilities. A Codex adapter could reuse the controller and provide:
...
- Existing host boundary: `src/host.ts`
- Current product decision: v0 keeps Codex activation deferred until an open, tested enforcement seam exists.
