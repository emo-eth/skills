...
test("OMP native TaskTool leaves ordinary children alone when wall-clock was never invoked", async () => {
  const root = mkdtempSync(join(tmpdir(), "wall-clock-omp-inactive-child-"));
  const created = await createMockTaskSession(root, [
    { content: [{ type: "toolCall", name: "read", arguments: { path: "README.md" } }] },
    { content: [{ type: "toolCall", name: "yield", arguments: { result: { data: { outcome: "ordinary child completed" } } } }] },
  ]);
  try {
    ...
    const result = await task!.execute("ordinary-task", {
      agent: "task",
      task: "Read README.md and report success",
    });

    expect(JSON.stringify(result)).toContain("ordinary child completed");
    expect(JSON.stringify(created.mock.calls)).not.toContain("Wall-clock blocked this child");
  } finally {
    await created.session.dispose();
    created.authStorage.close();
    rmSync(root, { recursive: true, force: true });
