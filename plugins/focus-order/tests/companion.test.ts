...
function install(host: FakeHost, source: "Pi" | "OMP" = "Pi"): FakeHost {
  installFocusOrderCompanion(host, source);
  return host;
}
...
function expectReported(
  ...
  assert.deepEqual(
    ...
    { key: "focus-order", value: `${source.toLowerCase()}:${status}` },
  );
...
test("registers a focus-order command with description and handler", () => {
  const host = makeFakeHost();
  install(host);
  const cmd = host.commands["focus-order"];
  assert.ok(cmd, "focus-order command should be registered");
  assert.equal(cmd.description, "Report this agent's focus-order waiting state");
...
for (const status of VALID_STATUSES) {
  test(`reports '${status}' status and returns the status string`, () => {
    const host = makeFakeHost();
    install(host);
    const message = host.commands["focus-order"].handler(status, CONTEXT) as string;
    assert.equal(message, `focus-order status: ${status}`);
  ...
  test(`translates source 'OMP' into a lowercase omp:${status} host value`, () => {
    const host = makeFakeHost();
    install(host, "OMP");
    host.commands["focus-order"].handler(status, CONTEXT);
...
test("trims surrounding whitespace around the status argument", () => {
  const host = makeFakeHost();
  install(host);
  host.commands["focus-order"].handler("  blocked  ", CONTEXT);
...
test("notifies with 'info' when reporting working", () => {
  const host = makeFakeHost();
  install(host);
  host.commands["focus-order"].handler("working", CONTEXT);
  assert.deepEqual(host.notifyCalls, [
    { message: "focus-order status: working", level: "info" },
  ]);
});
...
test("notifies with 'warning' when reporting non-working statuses", () => {
  ...
  host.commands["focus-order"].handler("blocked", CONTEXT);
  assert.deepEqual(host.notifyCalls, [
    { message: "focus-order status: blocked", level: "warning" },
  ]);
});
...
test("does not notify when host has no ui.notify", () => {
  ...
  const message = host.commands["focus-order"].handler("working", CONTEXT) as string;
  assert.equal(message, "focus-order status: working");
...
test("clear reports 'working' and updates host status and entry", () => {
  const host = makeFakeHost();
  install(host);
  const message = host.commands["focus-order"].handler("clear", CONTEXT) as string;
  assert.equal(message, "focus-order status: working");
  expectReported(host, "working", "Pi");
});
...
test("clear does not trigger a notification", () => {
  const host = makeFakeHost();
  install(host);
  host.commands["focus-order"].handler("clear", CONTEXT);
  assert.equal(host.notifyCalls.length, 0);
});
...
test("rejects an unknown status value", () => {
  ...
  install(host);
  assert.throws(
    () => host.commands["focus-order"].handler("bogus", CONTEXT),
    /focus-order status must be working, idle, blocked, or done/,
  );
...
test("accepts a status via the documented 'status <state>' form", () => {
  const host = makeFakeHost();
  install(host);
  const message = host.commands["focus-order"].handler("status working", CONTEXT) as string;
  assert.equal(message, "focus-order status: working");
  expectReported(host, "working", "Pi");
});
...
test("returns help string for an empty argument", () => {
  const host = makeFakeHost();
  install(host);
  const message = host.commands["focus-order"].handler("", CONTEXT) as string;
  assert.equal(message, "focus-order status <working|idle|blocked|done>; focus-order clear");
});
...
test("returns help string for 'help'", () => {
  const host = makeFakeHost();
  install(host);
  const message = host.commands["focus-order"].handler("help", CONTEXT) as string;
  assert.equal(message, "focus-order status <working|idle|blocked|done>; focus-order clear");
});
...
test("agent_end with a non-record event reports terminal 'done'", () => {
  ...
  install(host);
  const handler = host.handlers["agent_end"];
  assert.equal(handler(null, CONTEXT) as string, "focus-order status: done");
  assert.equal(handler("done", CONTEXT) as string, "focus-order status: done");
  assert.equal(handler([1, 2], CONTEXT) as string, "focus-order status: done");
...
test("report carries the session id from context into the appended entry", () => {
  const host = makeFakeHost();
  install(host);
  host.commands["focus-order"].handler("idle", { sessionId: "custom-session" });
  const entry = host.entryCalls.at(-1);
...
test("omits session_id when context has none", () => {
  const host = makeFakeHost();
  install(host);
  host.commands["focus-order"].handler("blocked", {});
  const entry = host.entryCalls.at(-1);
  assert.deepEqual(entry?.data, {
    source: "pi",
    status: "blocked",
...
test("appendEntry data carries source, status, and session id", () => {
  const host = makeFakeHost();
  install(host, "OMP");
  host.commands["focus-order"].handler("done", CONTEXT);
  const entry = host.entryCalls.at(-1);
  assert.deepEqual(entry?.data, {
    source: "omp",
    status: "done",
