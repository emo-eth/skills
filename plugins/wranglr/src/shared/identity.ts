...
export function rankOf(state: WranglrState, agent: AgentSnapshot): number | undefined {
...
export function isRanked(state: WranglrState, agent: AgentSnapshot): boolean {
  return rankOf(state, agent) !== undefined || worktreeRankOf(state, agent) !== undefined;
}

export function isSnoozed(state: WranglrState, agent: AgentSnapshot): boolean {
  const key = identityKey(identityFor(agent));
  return state.snoozed_agents.some((stored) => identityKey(stored.identity) === key);
}

export function orderedAgents(state: WranglrState, agents: AgentSnapshot[]): AgentSnapshot[] {
  return [...agents].sort((left, right) => compareAgents(state, left, right));
}

export function urgentAgents(state: WranglrState, agents: AgentSnapshot[]): AgentSnapshot[] {
...
export function addOrMoveToEnd(state: WranglrState, agent: AgentSnapshot): WranglrState {
...
export function unsetRank(state: WranglrState, agent: AgentSnapshot): WranglrState {
  const key = identityKey(identityFor(agent));
  return {
    ...state,
...
export function snoozeAgent(state: WranglrState, agent: AgentSnapshot): WranglrState {
  const identity = identityFor(agent);
  const key = identityKey(identity);
