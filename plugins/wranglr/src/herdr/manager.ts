...
async function main(): Promise<void> {
  ...
  let selection: Selection = preserveSelection(state, agents, { section: "agents" }, sort);

  const render = (): void => {
    const height = process.stdout.rows ?? 24;
    process.stdout.write(CLEAR + renderScreen(state, agents, selection, status, sort, height));
  };

  const reload = async (): Promise<void> => {
    agents = await listAgents();
    selection = preserveSelection(state, agents, selection, sort);
    status = `Loaded ${agents.length} agent pane(s)`;
    render();
  };

  const update = (next: WranglrState, message: string): void => {
    state = next;
    saveState(state);
    selection = preserveSelection(state, agents, selection, sort);
    status = message;
  ...
  const setSort = (next: SortState, message: string): void => {
    sort = next;
    selection = preserveSelection(state, agents, selection, sort);
    status = message;
    render();
  ...
  const handle = async (input: string): Promise<boolean> => {
    ...
    if (input === "?" || input === "h" || input === "H") {
      status = "Tab lists; arrows select; u/d rank; r add; x unset; s snooze; t sort column; T direction; f focus; m mode; e guard; q close";
      render();
      return true;
    }
    if (input === "\u001b[A" || input === "k") {
    ...
    if (input === "t") {
      const next: SortState = { key: nextSortKey(sort.key), dir: 1 };
      setSort(next, `Sort: ${sortColumnLabel(next.key)} ascending`);
      return true;
    }
    if (input === "T") {
      const next: SortState = { ...sort, dir: sort.dir === 1 ? -1 : 1 };
      setSort(next, `Sort: ${sortColumnLabel(next.key)} ${next.dir === 1 ? "ascending" : "descending"}`);
      return true;
    }
    if (input === "f" || input === "F") {
      await focusSelected();
  ...
  const moveSelection = (delta: -1 | 1): void => {
    const rows = selection.section === "agents"
      ? sortAgents(state, agents, sort).map((agent) => identityKey(identityFor(agent)))
      : worktreeRows(state, agents).map((row) => worktreeKey(row.identity));
    const key = moveSelectionKey(rows, selection.key, delta);
    selection = key === undefined ? { section: selection.section } : { section: selection.section, key };
  };

  const moveSelected = async (
    delta: -1 | 1,
    apply: (state: WranglrState, message: string) => void,
  ): Promise<void> => {
    if (selection.section === "agents") {
      const agent = selectedAgent(state, agents, selection, sort);
      if (!agent) return;
  ...
  const unsetSelected = (apply: (state: WranglrState, message: string) => void): void => {
    if (selection.section === "agents") {
      const agent = selectedAgent(state, agents, selection, sort);
      if (!agent) return;
...
const AGENT_SORT_COLUMNS: SortKey[] = ["rank", "status", "agent", "project", "worktree"];
...
function agentHeader(sort: SortState): string {
  return AGENT_SORT_COLUMNS.map((key) => {
    if (key === sort.key) {
      return `${sortColumnLabel(key).toUpperCase()}${sort.dir === 1 ? "^" : "v"}`;
    }
    return sortColumnLabel(key).toUpperCase();
  }).join("  ");
}
...
function renderScreen(
  ...
  rows.push(`${BOLD}Wranglr${RESET}  guard=${state.enabled ? `${GREEN}on${RESET}` : `${RED}off${RESET}`} mode=${state.mode}`);
  ...
  rows.push(`${DIM}${agentHeader(sort)}${RESET}`);
  const ordered = sortAgents(state, agents, sort);
  ...
  rows.push(`${DIM}q close  ? help  t sort${RESET}`);
  return `${viewportLines(rows, selectedLine, height).join("\n")}\n`;
}

function worktreeRows(state: WranglrState, agents: AgentSnapshot[]): WorktreeRow[] {
...
function preserveSelection(
  ...
  const rows = selection.section === "agents"
    ? sortAgents(state, agents, sort).map((agent) => identityKey(identityFor(agent)))
    : worktreeRows(state, agents).map((row) => worktreeKey(row.identity));
  return anchorSelection(rows, selection);
}
...
void main().catch((error: unknown) => {
  ...
  console.error(`wranglr manager stopped: ${message}`);
  process.exitCode = 1;
});
