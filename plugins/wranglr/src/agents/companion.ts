...
export function installWranglrCompanion(
  host: CompanionHost,
  source: "Pi" | "OMP",
): void {
  if (typeof host.registerCommand !== "function") {
    throw new Error(`Wranglr requires ${source}'s registerCommand hook`);
  }

  const report = (status: CompanionStatus, context: CompanionContext): string => {
    const value = `${source.toLowerCase()}:${status}`;
    host.setStatus?.("wranglr", value);
    host.appendEntry?.("wranglr-status", {
      source: source.toLowerCase(),
      status,
      session_id: context.sessionId,
    });
    return `wranglr status: ${status}`;
  };

  host.registerCommand("wranglr", {
    description: "Report this agent's wranglr waiting state",
    handler: (args, context) => {
      const tokens = args.trim().split(/\s+/).filter(Boolean);
      const requested = tokens[0] === "status" ? tokens[1] : tokens[0];
      if (!requested || requested === "help") {
        return "wranglr status <working|idle|blocked|done>; wranglr clear";
      }
      if (requested === "clear") return report("working", context);
      if (!isCompanionStatus(requested)) {
        throw new Error("wranglr status must be working, idle, blocked, or done");
      }
