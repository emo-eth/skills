...
import type { AgentIdentity, WranglrState } from "../src/shared/types.ts";
...
describe("normalized state is serializable and idempotent", () => {
  function roundtrip(st: WranglrState): WranglrState {
  ...
  it("is stable across an arbitrary number of passes", () => {
    ...
    for (let i = 0; i < 5; i += 1) {
      const again = roundtrip(st) as WranglrState;
