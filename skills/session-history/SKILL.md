...
- Use `session_history_expand` when a citation needs more surrounding transcript.
...
1. Call `session_history_resume` to recover the best-matching session packet.
2. Call `session_history_query` on the selected session for missing context.
3. Distinguish recorded unfinished work from your proposed next action.

## Lifecycle

Use `session_history_lifecycle` only when the user asks to exclude, disconnect, or purge sessions or sources. Destructive actions require explicit approval.
