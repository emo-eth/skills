...
4. Verify each component against its baseline with OMP `browser` screenshots and image comparison on the real surface. A nonzero diff fails. Iterate with the Autonomous run playbook until the diff is zero.
5. Run **Opening a PR** only when the user asked for a PR or shipping.
