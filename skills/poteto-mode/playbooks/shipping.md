...
1. **Verify every PR independently before arming anything.** Prepare one worktree per PR, then launch one verifier per PR in one OMP `task` batch. Each verifier did not write that PR and exercises the real surface against parent and head with OMP `browser`, the actual CLI/TUI, debugger evidence, or the service interface. Each returns `PASS`, `PASS+NOTES`, or `FAIL`. Post the verdict to its PR only when the user authorized shipping. CI green and bot approval are not verdicts.
...
8. **Watch the drain; do not drive it.** Start the queued watcher with OMP `hub start` over the verified run and wait for `COMPLETE` at the ceiling. `ADVANCE` is progress. Re-arm after any acted-on verdict. Report each merge and the new ceiling. Diagnose a stall before mutating the stack.
