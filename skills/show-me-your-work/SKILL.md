...
At the end, resolve the current OMP thread with `agent-conv thread <current-workspace> --source omp` when available. Otherwise inspect only the current workspace under `~/.omp/agent/sessions/`. Walk the log against what happened:

- Every row maps to a real action. Append a correction row for a false entry; preserve append-only history.
...
Before handing back, spawn one read-only OMP `reviewer` task that did not perform the work. The reviewer reads the audit trail and current transcript, then flags what the user should inspect. OMP does not guarantee a different model family, so report the agent type and do not claim cross-model review without proof.
...
Every reply for a run that produced a trail ends with an "Attention" section. Lead with `reviewed by <OMP agent type>`, then list flags that point to rows or transcript moments. "No flags" is valid. The self-audit checks truth; this review checks what still deserves scrutiny.
