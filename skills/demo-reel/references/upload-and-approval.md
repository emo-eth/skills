...
2. **Save locally** -- save to a stable OS-temp path ($TMPDIR/compound-engineering/demo-reel/)
...
Return the structured output defined in the SKILL.md Output section: `Tier`, `Description`, and either `URL` (permanent catbox URL) or `Path` (local file path). The caller formats the evidence into the PR description. demo-reel does not generate markdown.
