...
def append_records_safely(path: Path, records_to_append, mode=0o600):
    if not records_to_append:
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    data = "".join(json.dumps(r, ensure_ascii=False) + "\n" for r in records_to_append).encode("utf-8")
...
def migrate_files(input_paths, output_path: Path, backup: bool = True, dry_run: bool = False):
    ...
    try:
        ...
        os.close(lock_fd)
        # Note: lock file is NEVER unlinked to avoid breaking serialization for concurrent waiters
