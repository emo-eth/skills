...
Find the relevant code with OMP `glob` and `grep`, then use `read` and `lsp` definitions or references to trace the actual implementation. Do not guess from names.
