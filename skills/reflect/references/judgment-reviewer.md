...
- OMP `read` calls against `skill://<name>` or `SKILL.md` files under the source repo
- OMP `task` prompts that name a skill path or `skill://` URI
- Tool calls such as `bash`, `grep`, and MCP routes that match a skill's documented commands
