...
Use `## To get it moving` when the requested work has not started or a blocker prevents it from starting. Use `## To finish` when the agent has completed part of the requested work and the user must complete the remaining steps. For a complete task with one remaining external action, keep `What is going on`, `What I need from you`, and `## Finish line`; use `## Remaining external action` for the one required action and show the completion evidence. If no external action remains, omit the user-action sections. Include only sections that carry information; never pad an unblocked or complete task.
