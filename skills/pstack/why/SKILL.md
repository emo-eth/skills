...
If the target is vague, infer the best referent from the conversation, recent edits, and active file or symbol. State the interpretation briefly so the user can redirect, then proceed.
...
Before spawning investigators, read the OMP system context's available-tools map and `MCP Tool Routes`. Those lists are authoritative. Do not inspect hidden application directories or guess that an MCP exists.
...
Launch all matching investigators in one OMP `task` batch so they run concurrently. One investigator per category owns one tool or MCP. Use `why-investigators` from `~/.config/pstack/omp-agents.json` when present, or `scout` by default. Every investigator is read-only. If a configured type is unavailable, fall back to `scout`.
...
Spawn one read-only synthesizer task after all investigators finish. Use `why-synthesizer` from `~/.config/pstack/omp-agents.json` when present, or `reviewer` by default. Pass large investigator results through a `local://` artifact. The synthesizer can spot-check citations with available MCP tools but writes no files.
