...
Use `agent-conv read <current-workspace> --source omp` to list OMP threads and `agent-conv thread <current-workspace> --source omp --session <id>` to read one in full. Use the exact current directory as the workspace query. If unavailable, inspect only the matching workspace under `~/.omp/agent/sessions/`.
...
3. Fan out across chat history. Use `agent-conv search <topic>` to narrow candidates, then spawn one `scout` task per independent time slice in one OMP `task` batch. Each scout reads only the selected OMP threads and returns one block per chat: topic, user goal, decisions, open threads, struggles and corrections, and artifacts, with the session ID. For one or two chats, skip fan-out and read them directly. Raw transcripts stay outside the main context; the main thread keeps findings only.
