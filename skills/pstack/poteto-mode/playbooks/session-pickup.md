...
**You own the resume point. Read the prior trail; do not redo it.** For "take over this", "resume this conversation", an OMP session ID, `agent://` or `history://` handoff, or a pushed branch.
...
1. Resolve the named OMP thread with `agent-conv thread` when available, or read a supplied `agent://` or `history://` URI. Stay inside the named workspace. Read metadata and final turns first, then decision points. Give a long transcript to one read-only `scout` and keep only its reduced timeline in the main thread.
