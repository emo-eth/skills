...
1. Capture a baseline trace on the real surface. Use OMP `debug`, a platform profiler, or the actual program's tracing interface.
...
3. Plan the fix from the trace. If it crosses a function boundary, run `architect`. Delegate only the scoped implementation, using `perf-issue` from `~/.config/pstack/omp-agents.json` or the default task agent. Review the diff and capture a post-fix trace.
   Apply the **sequence-verifiable-units** principle skill, verifying each attempt before trying the next.
4. Parse and compare the artifacts (JSON to sqlite, diff). "Inconclusive" or wrong-surface is not a pass; flag it.
5. Cite the measurement in the final report and in the PR when one is requested.
